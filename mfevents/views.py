...
import pytz
from .models import MFEvent, EventEnquiry, Mix
...
def event_profile(request, event_id):
    event = MFEvent.objects.get(pk=event_id)
    ...
    bands = event.bands.exclude(name='N/A')
    venue_url = event.venue.website
    mixes = Mix.objects.filter(event=event_id).order_by('name')

    genres_vl = event.genre.exclude(name='N/A').values_list('name', flat=True)
...
def enquiry_success(request):
    ...
    send_mail(
        ...
        'msg: ' + enq.message + '\nfrom ' + enq.name + '\nreturn email: '+ enq.email, # message
        enq.email, # from email
        ['morefireenq@gmail.com'], #To email
    )
...
def events_list(request):
    ...
    events_list = MFEvent.objects.exclude(status='Plan').exclude(finished=True).order_by('start_dt')
...
def past_events_list(request):
    ...
    events_list_past = MFEvent.objects.filter(finished=True).filter(status='Scheduled').order_by('-finish_dt')
